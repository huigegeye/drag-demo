  ...
  <div class="container">
    ...
    <div class="section">
      ...
      <div class="stage"
        ...
        <div class="status-toast"
             v-if="imageState === 'noimage'">暂无图片</div>
        <!-- 点击效果 -->
        <transition name="fade">
          <div v-if="trippleVisible"
               class="tripple"
               :style="trippleStyle"></div>
        </transition>
        <DragImage :url="imageUrl"
          ...
          <template #default="{ imageW, imageH, scale }">
            <vue-drag-custom v-for="point in points"
              ...
              <!-- 自定义节点 -->
              <span class="dot"
                    @click="alter($event)">{{ point.code }}</span>
            </vue-drag-custom>
          </template>
        </DragImage>
      </div>
